 ...
 *  Author: Robbert Berrevoets, Raja Ramlal en Victor Hogeweij
...
long afstand =  1; //Hier wordt de afstand in opgeslagen
volatile char knopstatus = 0; //Hier wordt de laatst ingedrukte knop in opgeslagen
//volatile long tags[3] = {2,4,6};
bool startConditie, stopConditie = false;
static double nauwkeurig = 1;
...
/*
 * Functie: Flank_Encoder
 * Description: Geeft de huidige afstand door m.b.v. de flank van de encoder
 * Parameters: Geen
 * Return: NULL
...
__interrupt void Flank_Encoder(void)
{
    ...
    nauwkeurig += 10.5;                             // incrementeer de afstand met 20cm omdat er 2 gaten zijn
    afstand = (int) nauwkeurig;                             // Double omzetten naar int zodat er geen kommagetallen op het display komen
    P6IFG &= ~BIT0;                                         // interrupt reset
}
...
void INIT_interfaces(){
    ...
    //
    //Interrupt instellen voor P6.0 (encoder)
    P6REN |= BIT0; // pull-down weerstand instellen
    P6OUT &= ~BIT0;
    P6IES &= ~(1<<0); //P1IES Flank laag naar hoog.
    P6IE |= 1<<0;     //P1IE Interrupt enable.
    //
